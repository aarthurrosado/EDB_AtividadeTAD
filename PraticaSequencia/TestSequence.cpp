...
void testAll()
{
    ...
    // Testa inserção e remoção básica
...
void testRemoveEnd()
{
    ...
    sequence->insertBegin("c");
    assert( sequence->getQuantity() == 2 );
    assert( sequence->get(1) == "c" ); // corrigido
    assert( sequence->get(2) == "b" ); // corrigido
    
    assert( sequence->removeEnd() == "b" ); // agora coerente
    assert( sequence->getQuantity() == 1 );
